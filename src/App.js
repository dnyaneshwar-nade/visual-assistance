...
import React, { useState, useRef, useEffect } from 'react';
import { Camera, Mic, Volume2, VolumeX } from 'lucide-react';
import { analyzeImage, captureAndProcess, speakText } from './services/gemini';
...
const App = () => {
    ...
    if (file) {
      ...
      reader.onload = async (e) => {
        ...
        try {
          ...
          speakText(analysisResult); // Optionally speak the result
        } catch (err) {
          setError('Upload analysis error: ' + err.message);
        } finally {
          setIsProcessing(false);
        }
      };
  ...
  return (
    <div className="flex flex-col items-center max-w-lg mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Visual Learning Assistant</h1>
      <p className="text-gray-600 mb-6">
        Point your camera at any problem or upload an image for instant help and explanation
      </p>
      ...
      <div className="flex gap-4 mb-4">
        ...
        {/* Upload and Analyze */}
        <label className="flex items-center gap-2 px-4 py-2 rounded bg-purple-500 hover:bg-purple-600 text-white cursor-pointer">
          <input
            type="file"
            accept="image/*"
